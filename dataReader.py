...
def read_dataset(path, one_class_img=4):
    """
    读取包含图片名和标签的csv
    :param csv_path:
...
def process_one_task(one_task, width=cfg.width, height=cfg.height, channel=cfg.channel):
    """
...
def task_split(classes: list, q_query=1, n_way=2, k_shot=1):
    """
    将各个分类下的img_path，按任务为单位分类。这个API是基于Mini-ImageNet下实现的，其中每个类只有600个
    为了均匀利用到所有数据，(q_query + k_shot) * n_way 要能被 图片数量整除
    。n-way * k-shot张图片用来给inner loop训练，n-way * query是给out loop去test
